...
class YellowDot {
  constructor() {
    ...
    // Pre-generate sub-particle offsets for nutrient cluster
    this.subParticles = [];
    for (let i = 0; i < 4; i++) {
  ...
  draw() {
    ...
    const pulseSpeed = (typeof PETRI_CONFIG !== 'undefined') ? PETRI_CONFIG.nutrientPulseSpeed : 0.04;
    const jitter = (typeof PETRI_CONFIG !== 'undefined') ? PETRI_CONFIG.nutrientJitter : 1.5;
    const pulse = 0.6 + 0.4 * Math.sin(frameCount * pulseSpeed + this.pulseOffset);

    // Outer glow (additive)
    const prevOp = ctx.globalCompositeOperation;
    ctx.globalCompositeOperation = 'lighter';
    const grad = ctx.createRadialGradient(screen.x, screen.y, 0, screen.x, screen.y, size * 3);
    grad.addColorStop(0, `rgba(180, 220, 60, ${0.12 * pulse})`);
    grad.addColorStop(1, 'rgba(180, 220, 60, 0)');
    ctx.fillStyle = grad;
    ctx.beginPath();
    ...
    // Central bright dot
    ctx.fillStyle = `rgba(200, 235, 70, ${0.7 * pulse})`;
    ctx.beginPath();
    ctx.arc(screen.x, screen.y, size * 0.8, 0, Math.PI * 2);
    ctx.fill();

    // Sub-particles (jittering cluster)
    for (const sp of this.subParticles) {
      const jx = sp.ox + (Math.random() - 0.5) * jitter;
      const jy = sp.oy + (Math.random() - 0.5) * jitter;
      ctx.fillStyle = `rgba(220, 245, 100, ${0.5 * pulse})`;
      ctx.beginPath();
      ctx.arc(screen.x + jx * scale, screen.y + jy * scale, size * sp.size, 0, Math.PI * 2);
      ctx.fill();
    }
  }
}
...
class Bot {
  ...
  update() {
    ...
    // Record trail position for petri dish visualization
    if (this.trail) {
      this.trail.push({ x: this.x, y: this.y });
      const maxLen = (typeof PETRI_CONFIG !== 'undefined') ? PETRI_CONFIG.trailLength : 120;
