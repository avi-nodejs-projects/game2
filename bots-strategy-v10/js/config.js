// Bots Strategy v10 - Configuration and Constants
...
// ============ PLAYER STATS (legacy single-player, kept for backwards compatibility) ============
const playerStats = {
  speed: 5,
  attack: 5,
  defence: 5,
...
// ============ DEFAULT PLAYER CONFIG TEMPLATE ============
const DEFAULT_PLAYER_CONFIG = {
  ...
  strategyMode: 'simple',
  behaviorWeights: null, // Will be initialized from BEHAVIORS
  rules: null, // Will be initialized from DEFAULT_RULES
  states: null, // Will be initialized from DEFAULT_STATES
  transitions: null, // Will be initialized from DEFAULT_TRANSITIONS
  currentStateId: 'gathering'
};

// ============ TWO-PLAYER CONFIGURATION ============
// playerConfigs[0] = Player 1, playerConfigs[1] = Player 2
const playerConfigs = [
  {
    id: 0,
    name: 'Player 1',
    ...
    preferredBonusStat: 'speed',
    strategyMode: 'simple',
    behaviorWeights: null, // Initialized in initPlayerConfigs()
    rules: null,
    states: null,
    transitions: null,
    currentStateId: 'gathering'
...
// Helper functions for two-player configuration
function getPlayerConfig(index) {
  if (index < 0 || index >= playerConfigs.length) {
...
function setPlayerConfig(index, config) {
  if (index < 0 || index >= playerConfigs.length) {
    console.warn(`Invalid player index: ${index}`);
    return;
  }
  Object.assign(playerConfigs[index], config);
}

function getBotPlayerConfig(bot) {
  if (bot.isPlayer && bot.playerIndex !== null && bot.playerIndex !== undefined) {
    // Bounds check to prevent accessing undefined index
    if (bot.playerIndex >= 0 && bot.playerIndex < playerConfigs.length) {
      return playerConfigs[bot.playerIndex];
    }
    console.warn(`Invalid bot.playerIndex: ${bot.playerIndex}`);
    return playerConfigs[0]; // Fallback to first player config
  }
  return null; // NPC
}

// Sync playerConfigs[0] with legacy global state for backwards compatibility
function syncPlayerConfigToLegacy(playerIndex = 0) {
  const config = playerConfigs[playerIndex];
  if (playerIndex === 0) {
    playerStats.speed = config.stats.speed;
    playerStats.attack = config.stats.attack;
    playerStats.defence = config.stats.defence;
    playerStats.lives = config.stats.lives;
    preferredBonusStat = config.preferredBonusStat;
    strategyMode = config.strategyMode;

    // Sync behavior weights
    if (config.behaviorWeights) {
      Object.keys(config.behaviorWeights).forEach(key => {
        if (behaviorWeights[key]) {
          behaviorWeights[key] = JSON.parse(JSON.stringify(config.behaviorWeights[key]));
        }
      });
    }
    ...
    if (config.rules) {
      rules.length = 0;
      config.rules.forEach(r => rules.push(JSON.parse(JSON.stringify(r))));
    }

    // Sync states and transitions
    if (config.states) {
      states.length = 0;
      config.states.forEach(s => states.push(JSON.parse(JSON.stringify(s))));
    }
    if (config.transitions) {
      transitions.length = 0;
      config.transitions.forEach(t => transitions.push(JSON.parse(JSON.stringify(t))));
    }
    currentStateId = config.currentStateId;
  }
}

// Sync legacy global state to playerConfigs[0]
function syncLegacyToPlayerConfig(playerIndex = 0) {
  if (playerIndex === 0) {
    playerConfigs[0].stats.speed = playerStats.speed;
    playerConfigs[0].stats.attack = playerStats.attack;
    playerConfigs[0].stats.defence = playerStats.defence;
    playerConfigs[0].stats.lives = playerStats.lives;
    playerConfigs[0].preferredBonusStat = preferredBonusStat;
    playerConfigs[0].strategyMode = strategyMode;
    playerConfigs[0].behaviorWeights = JSON.parse(JSON.stringify(behaviorWeights));
    playerConfigs[0].rules = JSON.parse(JSON.stringify(rules));
    playerConfigs[0].states = JSON.parse(JSON.stringify(states));
    playerConfigs[0].transitions = JSON.parse(JSON.stringify(transitions));
    playerConfigs[0].currentStateId = currentStateId;
  }
}
...
// This must be called after BEHAVIORS, DEFAULT_RULES, DEFAULT_STATES, DEFAULT_TRANSITIONS are defined
function initPlayerConfigs() {
  // Create default behavior weights
  function createDefaultBehaviorWeights() {
    const weights = {};
    Object.keys(BEHAVIORS).forEach(key => {
      const params = {};
      if (BEHAVIORS[key].params) {
  ...
  // Initialize both player configs with default values
  for (let i = 0; i < playerConfigs.length; i++) {
    playerConfigs[i].behaviorWeights = createDefaultBehaviorWeights();
    playerConfigs[i].rules = JSON.parse(JSON.stringify(DEFAULT_RULES));
    playerConfigs[i].states = JSON.parse(JSON.stringify(DEFAULT_STATES));
    playerConfigs[i].transitions = JSON.parse(JSON.stringify(DEFAULT_TRANSITIONS));
    playerConfigs[i].currentStateId = 'gathering';
  }

  // Set player-specific defaults
  playerConfigs[0].name = 'Player 1';
  playerConfigs[0].color = '#FFD700'; // Gold
  playerConfigs[1].name = 'Player 2';
  playerConfigs[1].color = '#00CED1'; // Turquoise
}
...
function resetPlayerConfigs() {
  ...
  for (let i = 0; i < playerConfigs.length; i++) {
    playerConfigs[i].stats = { speed: 5, attack: 5, defence: 5, lives: 3 };
    playerConfigs[i].preferredBonusStat = 'speed';
    playerConfigs[i].strategyMode = 'simple';
    playerConfigs[i].behaviorWeights = createDefaultBehaviorWeights();
    playerConfigs[i].rules = JSON.parse(JSON.stringify(DEFAULT_RULES));
    playerConfigs[i].states = JSON.parse(JSON.stringify(DEFAULT_STATES));
    playerConfigs[i].transitions = JSON.parse(JSON.stringify(DEFAULT_TRANSITIONS));
  ...
  playerConfigs[0].name = 'Player 1';
  playerConfigs[0].color = '#FFD700';
  playerConfigs[1].name = 'Player 2';
  playerConfigs[1].color = '#00CED1';
}
