...
function evaluatePackFormation() {
  ...
  // Track proximity pairs
  const proximityMap = new Map(); // `${i}-${j}` -> frames together
  ...
  for (let i = 0; i < bots.length; i++) {
    for (let j = i + 1; j < bots.length; j++) {
      ...
      if (bot1.relationships?.packId !== null && bot2.relationships?.packId !== null) {
      ...
      if (dist <= cfg.formation.proximityDistance) {
        ...
        if (similarity >= cfg.formation.similarityThreshold) {
          const key = `${i}-${j}`;
          // evaluatePackFormation runs every 60 frames, so increment by 60 to match real frame count
          const currentFrames = (bot1.packProximityMap?.get(bot2.index) || 0) + 60;
...
function updatePacks() {
  ...
  for (const [packId, pack] of packs.entries()) {
    ...
    if (cfg.bonds.disbandOnStarvation) {
      const members = getPackMembers(pack);
      if (members.length === 0) { disbandPack(pack, 'allMembersGone'); continue; }
      ...
      if (starvingRatio >= cfg.bonds.starvationDisbandThreshold) {
        // Disband the entire pack (not just kick starving members), matching the label
        disbandPack(pack, 'starvation');
        continue;
      }
    }
