...
Bot.prototype.draw = function(rc, isFollowed = false) {
  if (!rc.isVisible(this.x, this.y, 150)) return;
  ...
  // Health bar (thin bar above bot)
  const healthBarWidth = size * 2.5;
  const healthBarHeight = 3 * scale;
  const healthBarY = screen.y - size - 16 * scale;
  const healthRatio = Math.max(0, Math.min(1, this.lives / (this.initialLives || 3)));
  // Background
  ctx.fillStyle = 'rgba(0,0,0,0.4)';
  ctx.fillRect(screen.x - healthBarWidth / 2, healthBarY, healthBarWidth, healthBarHeight);
  // Fill — green > yellow > red
  let barColor;
  if (healthRatio > 0.6) barColor = '#66bb6a';
  else if (healthRatio > 0.3) barColor = '#ffa726';
  else barColor = '#ef5350';
  ctx.fillStyle = barColor;
  ctx.fillRect(screen.x - healthBarWidth / 2, healthBarY, healthBarWidth * healthRatio, healthBarHeight);

  // Strategy badge (small letter below bot)
  if (!this.isPlayer && this.npcStrategy) {
    const badgeLetter = this.npcStrategy.charAt(0).toUpperCase();
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.font = `${8 * scale}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(badgeLetter, screen.x, screen.y + size + 10 * scale);
  }

  // Lives indicator
  const livesY = screen.y - size - 22 * scale;
