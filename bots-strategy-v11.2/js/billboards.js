// Bots Strategy v9 - Billboard System
...
function spawnBillboard() {
  ...
  // Position billboard slightly offset from cluster center
  const angle = Math.random() * Math.PI * 2;
  const distance = Math.random() * billboardSettings.clusterProximityRadius * 0.5;
...
function updateBillboards() {
  // Always age/expire existing billboards, even when disabled, so they don't
  // freeze in place and re-appear stale when re-enabled
...
function drawBillboards(rc) {
  ...
  for (const bb of sortedBillboards) {
    bb.draw(rc.ctx, rc.worldToScreen.bind(rc), rc.getScale.bind(rc), rc.isVisible.bind(rc));
  }
}
