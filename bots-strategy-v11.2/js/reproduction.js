// Bots Strategy v9 - Reproduction System
...
function isMature(bot) {
  ...
  switch (cfg.maturityMetric) {
    case 'frames':
      // Use bot.lifetime (always-incrementing) so maturity works even when age feature is disabled
      return (bot.lifetime || 0) >= cfg.maturityThreshold;
    case 'stats': {
    ...
    case 'hybrid': {
      const hybridScore = (bot.lifetime || 0) / 600 + (bot.killCount || 0) * 10;
...
function updateMatingProgress(bot1, bot2) {
  ...
  if (dist <= cfg.proximityDistance) {
    // Increment mating progress — pack mates progress faster by packBonus.weight
    const packBonusCfg = cfg.packBonus;
    const progressIncrement = (packBonusCfg.enabled && typeof arePackMates === 'function' && arePackMates(bot1, bot2))
      ? (packBonusCfg.weight || 2)
      : 1;
...
function calculateOffspringStatsSexual(parent1, parent2) {
  ...
  // Normalize to prevent runaway stat inflation across generations (same cap as asexual)
  const targetTotal = TOTAL_POINTS;
  const currentTotal = stats.speed + stats.attack + stats.defence + stats.lives;
  if (currentTotal > targetTotal * 1.5) {
    const scale = (targetTotal * 1.5) / currentTotal;
...
function createOffspring(parent1, parent2, stats, position) {
  // Use monotonic nextBotIndex to avoid collisions when bots are spliced out of the array
  const newIndex = nextBotIndex++;
  const offspring = new Bot(newIndex, false);

  // Clear any NPC strategy assigned by the constructor — inheritStrategy sets it correctly
  offspring.npcStrategy = null;
  offspring.npcBehaviors = null;
  offspring.npcWeights = null;
  ...
  switch (cfg.method) {
    ...
    default:
      ...
      Object.keys(BEHAVIORS).forEach(key => {
        // Player bot uses global behaviorWeights, not npcWeights
        const w1 = parent1.isPlayer ? (behaviorWeights[key]?.weight || 0) : (parent1.npcWeights?.[key] || 0);
        const w2 = parent2 ? (parent2.isPlayer ? (behaviorWeights[key]?.weight || 0) : (parent2.npcWeights?.[key] || 0)) : w1;
...
function checkAsexualReproduction() {
  ...
  // Snapshot to avoid visiting newly-spawned offspring in the same pass
  const snapshot = bots.slice();
