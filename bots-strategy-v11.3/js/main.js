// Bots Strategy v11.3 "The Arena" - Main Game Loop
...
function worldToScreen(worldX, worldY) {
  const z = camera.zoom;
  return {
    x: (worldX - camera.x) * z + canvas.width / 2,
    y: (worldY - camera.y) * z + canvas.height / 2
  };
}
...
function getScale(worldY) {
  ...
  return Math.max(0.5, Math.min(1.5, scale)) * camera.zoom;
}

// Mouse hover tracking for arena name tags
let hoveredBot = null;
...
function handleCombat(bot1, bot2) {
  ...
  if (damage1 <= 0 && damage2 <= 0) {
    ...
    // Arena: combat contact event
    if (arenaConfig.enabled && typeof arenaEvents !== 'undefined') {
      arenaEvents.emit('combat:contact', { x: (bot1.x + bot2.x) / 2, y: (bot1.y + bot2.y) / 2, bot1, bot2, damage1, damage2 });
    }
...
function handleBotDeath(deadBot, killerBot) {
  // Arena: kill event
  if (arenaConfig.enabled && typeof arenaEvents !== 'undefined') {
    arenaEvents.emit('combat:kill', {
      x: deadBot.x, y: deadBot.y,
      dead: deadBot, killer: killerBot,
      isPlayerKill: killerBot && killerBot.isPlayer,
      isPlayerDeath: deadBot.isPlayer
    });
  }
  ...
  for (const bot of bots) {
    for (const dot of yellowDots) {
      if (checkBotDotCollision(bot, dot)) {
        ...
        // Arena: wilt vegetation for consumed dot
        if (arenaConfig.enabled && vegetationState.length > 0) {
          const dotIdx = yellowDots.indexOf(dot);
          if (dotIdx >= 0 && vegetationState[dotIdx]) {
            vegetationState[dotIdx].wiltProgress = 1;
            vegetationState[dotIdx].regrowing = false;
            // Start regrow after a delay
            setTimeout(() => {
              if (vegetationState[dotIdx]) vegetationState[dotIdx].regrowing = true;
            }, 1000);
          }
        }
        dot.respawn();
...
let wearCanvas = null, wearCtx = null;
let scorchCanvas = null, scorchCtx = null;
let vegetationState = [];
let starPositions = []; // seeded once

function getDayPhase(frame) {
  const cycle = arenaConfig.dayNight.cycleDuration;
...
function generateTerrainTexture() {
  const size = arenaConfig.terrain.textureSize;
  terrainCanvas = document.createElement('canvas');
  terrainCanvas.width = size;
  terrainCanvas.height = size;
  terrainCtx = terrainCanvas.getContext('2d');
  const grad = terrainCtx.createLinearGradient(0, 0, 0, size);
  grad.addColorStop(0, arenaConfig.terrain.baseColor1);
  grad.addColorStop(1, arenaConfig.terrain.baseColor2);
  terrainCtx.fillStyle = grad;
  terrainCtx.fillRect(0, 0, size, size);
  for (let i = 0; i < 2000; i++) {
    const x = Math.random() * size;
    const y = Math.random() * size;
    const r = 3 + Math.random() * 15;
    const light = Math.random() > 0.5;
    terrainCtx.fillStyle = light
      ? `rgba(120, 180, 80, ${0.05 + Math.random() * 0.1})`
      : `rgba(40, 70, 30, ${0.05 + Math.random() * 0.1})`;
...
function initWearMap() {
  wearCanvas = document.createElement('canvas');
  wearCanvas.width = 400;
  wearCanvas.height = 400;
  wearCtx = wearCanvas.getContext('2d');
}

function initScorchMap() {
  scorchCanvas = document.createElement('canvas');
  scorchCanvas.width = 400;
  scorchCanvas.height = 400;
  scorchCtx = scorchCanvas.getContext('2d');
}

function updateWearMap() {
  if (!wearCtx) return;
  const sx = 400 / WORLD_WIDTH;
  const sy = 400 / WORLD_HEIGHT;
  wearCtx.globalCompositeOperation = 'destination-in';
  wearCtx.fillStyle = 'rgba(255,255,255,0.9995)';
  wearCtx.fillRect(0, 0, 400, 400);
  wearCtx.globalCompositeOperation = 'source-over';
  for (const bot of bots) {
    wearCtx.fillStyle = 'rgba(60, 40, 20, 0.015)';
    wearCtx.beginPath();
    wearCtx.arc(bot.x * sx, bot.y * sy, 3, 0, Math.PI * 2);
    wearCtx.fill();
  }
}

function stampScorchMark(worldX, worldY) {
  if (!scorchCtx) return;
  const sx = 400 / WORLD_WIDTH;
  const sy = 400 / WORLD_HEIGHT;
  scorchCtx.fillStyle = 'rgba(40, 20, 10, 0.3)';
  scorchCtx.beginPath();
  scorchCtx.arc(worldX * sx, worldY * sy, 6, 0, Math.PI * 2);
  scorchCtx.fill();
}

function decayScorchMap() {
  if (!scorchCtx) return;
  scorchCtx.globalCompositeOperation = 'destination-in';
  scorchCtx.fillStyle = 'rgba(255,255,255,0.9998)';
  scorchCtx.fillRect(0, 0, 400, 400);
  scorchCtx.globalCompositeOperation = 'source-over';
}

function initVegetation() {
  vegetationState = yellowDots.map((dot, i) => ({
    dotIndex: i,
    tufts: Array.from({ length: arenaConfig.vegetation.tuftsPerDot }, () => ({
...
function drawVegetation() {
  for (const veg of vegetationState) {
    const dot = yellowDots[veg.dotIndex];
    if (!dot || !isVisible(dot.x, dot.y, 50)) continue;
    const screen = worldToScreen(dot.x, dot.y);
    const scale = getScale(dot.y);
    const wilt = veg.wiltProgress;
    for (const tuft of veg.tufts) {
      const h = tuft.height * (1 - wilt * 0.6) * scale;
      const hue = 100 + wilt * 40;
      const sat = 60 - wilt * 30;
      ctx.strokeStyle = `hsl(${hue}, ${sat}%, ${35 + wilt * 15}%)`;
      ctx.lineWidth = 1.5 * scale;
      ctx.beginPath();
      const bx = screen.x + tuft.offsetX * scale;
      const by = screen.y + tuft.offsetY * scale;
      ctx.moveTo(bx, by);
      ctx.lineTo(bx + Math.sin(tuft.angle) * h * 0.3, by - h);
      ctx.stroke();
    }
  }
}
...
// ============ DRAWING ============
function drawField() {
  if (!arenaConfig.enabled) { drawFieldOriginal(); return; }

  const dayPhase = getDayPhase(frameCount);
  const phaseColors = arenaConfig.sky[dayPhase.phase];

  // Sky
  const skyGrad = ctx.createLinearGradient(0, 0, 0, canvas.height * 0.3);
  skyGrad.addColorStop(0, phaseColors.skyTop);
  skyGrad.addColorStop(1, phaseColors.skyBottom);
  ctx.fillStyle = skyGrad;
  ctx.fillRect(0, 0, canvas.width, canvas.height * 0.3);

  // Stars at night/predawn
  if (dayPhase.phase === 'night' || dayPhase.phase === 'predawn') {
    const starAlpha = dayPhase.phase === 'night' ? 0.8 : (1 - dayPhase.blend) * 0.6;
    ctx.fillStyle = `rgba(255, 255, 255, ${starAlpha})`;
    for (const star of starPositions) {
      const twinkle = 0.5 + 0.5 * Math.sin(frameCount * 0.02 + star.twinkle);
  ...
  // Terrain texture tiled across visible area
  if (terrainCanvas) {
    const rect = getWorldScreenRect();
    ctx.drawImage(terrainCanvas, 0, 0, terrainCanvas.width, terrainCanvas.height, rect[0], rect[1], rect[2], rect[3]);
  } else {
    // Fallback gradient
  ...
  if (wearCanvas) {
    ctx.globalAlpha = 0.4;
    const rect = getWorldScreenRect();
    ctx.drawImage(wearCanvas, 0, 0, 400, 400, rect[0], rect[1], rect[2], rect[3]);
    ctx.globalAlpha = 1.0;
  }
  ...
  if (scorchCanvas) {
    ctx.globalAlpha = 0.5;
    const rect = getWorldScreenRect();
    ctx.drawImage(scorchCanvas, 0, 0, 400, 400, rect[0], rect[1], rect[2], rect[3]);
    ctx.globalAlpha = 1.0;
  }

  // Day/night field tint
  ctx.fillStyle = phaseColors.fieldTint;
  ctx.fillRect(0, canvas.height * 0.2, canvas.width, canvas.height * 0.8);
...
function animate() {
  ...
  // Arena: freeze frame — skip game logic but still render + update particles
  const arenaActive = arenaConfig && arenaConfig.enabled;
  if (arenaActive && typeof shouldSkipGameUpdate === 'function' && shouldSkipGameUpdate()) {
    // Still update visual systems during freeze
    updateCamera();
    if (typeof particlePool !== 'undefined') particlePool.update();
    if (typeof updateDamageNumbers === 'function') updateDamageNumbers();
    if (typeof updateImpactRings === 'function') updateImpactRings();
    // Full draw pass (below) still happens
  } else {
  ...
  // Arena: director camera
  if (arenaActive && typeof updateDirector === 'function') updateDirector();
  if (arenaActive && typeof detectApproachPhase === 'function') detectApproachPhase();
  ...
  if (arenaActive && typeof applyScreenShake === 'function') {
    shakeOffset = applyScreenShake();
  ...
  // Arena: particles, effects, tension lines
  if (arenaActive) {
    if (typeof particlePool !== 'undefined') particlePool.draw(ctx, worldToScreen, getScale);
    if (typeof drawImpactRings === 'function') drawImpactRings();
    if (typeof drawTensionLines === 'function') drawTensionLines();
  ...
  // Arena: danger indicators, kill feed, stat popups
  if (arenaActive) {
    if (typeof drawDangerIndicators === 'function') drawDangerIndicators();
    if (typeof drawKillFeed === 'function') drawKillFeed();
    if (typeof drawStatPopups === 'function') drawStatPopups();
  }
  ...
  // Arena: vignette (drawn on top, not affected by shake)
  if (arenaActive && typeof drawVignette === 'function') {
    // Adjust vignette based on followed bot health
    if (camera.followBot) {
      const healthRatio = camera.followBot.lives / (camera.followBot.initialLives || 3);
      if (healthRatio < 0.3 && typeof setVignetteIntensity === 'function') {
        setVignetteIntensity(0.4, 0.2);
      }
    }
...
function runGameUpdate() {
  ...
  // Arena updates
  if (arenaConfig && arenaConfig.enabled) {
    if (frameCount % 3 === 0) updateWearMap();
    decayScorchMap();
    if (typeof particlePool !== 'undefined') particlePool.update();
    if (typeof updateDamageNumbers === 'function') updateDamageNumbers();
    if (typeof updateImpactRings === 'function') updateImpactRings();
    // Update vegetation wilt/regrow
    for (const veg of vegetationState) {
      if (veg.wiltProgress > 0 && !veg.regrowing) {
        // Stay wilted for a bit, then start regrowing
      } else if (veg.regrowing) {
        veg.wiltProgress = Math.max(0, veg.wiltProgress - 1 / arenaConfig.vegetation.regrowFrames);
        if (veg.wiltProgress <= 0) veg.regrowing = false;
      }
    }
    // Ambient particles
    if (frameCount % 3 === 0 && typeof particlePool !== 'undefined' && particlePool.activeCount < 250) {
      const dayPhase = getDayPhase(frameCount);
      const wx = camera.x + (Math.random() - 0.5) * canvas.width / camera.zoom;
      const wy = camera.y + (Math.random() - 0.5) * canvas.height / camera.zoom;
      if (dayPhase.phase === 'day') {
        particlePool.emit(wx, wy, 1, 'DUST');
      } else if (dayPhase.phase === 'dawn' || dayPhase.phase === 'dusk') {
        particlePool.emit(wx, wy, 1, 'FIREFLY');
      }
    }
  }
}
...
function startGame() {
  ...
  if (arenaConfig && arenaConfig.enabled) {
    ...
    initVegetation();
    initStars();
    if (typeof particlePool !== 'undefined') particlePool.init(arenaConfig.particles.poolSize);
    if (typeof initVignette === 'function') initVignette(canvas.width, canvas.height);
    if (typeof initArenaEventListeners === 'function') initArenaEventListeners();
  }
...
function initKeyboardControls() {
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'c' || e.key === 'C') {
      if (typeof toggleDirector === 'function') toggleDirector();
    }
  });
}
...
function init() {
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    let minDist = 30;
    for (const bot of bots) {
      const dx = bot.x - mouseWorldX;
      const dy = bot.y - mouseWorldY;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist < minDist) { minDist = dist; hoveredBot = bot; }
    }
  });
  canvas.addEventListener('mouseleave', () => { hoveredBot = null; });
