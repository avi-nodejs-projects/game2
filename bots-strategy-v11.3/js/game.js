...
class Bot {
  constructor(index, isPlayer = false) {
    ...
    this.posHistory = []; // Arena: position history for motion blur trail
  ...
  update() {
    ...
    // Arena: track position history for motion blur
    if (typeof arenaConfig !== 'undefined' && arenaConfig.enabled) {
      this.posHistory.unshift({ x: this.x, y: this.y });
      if (this.posHistory.length > 3) this.posHistory.length = 3;
    }
