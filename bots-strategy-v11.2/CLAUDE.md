...
v11.2 "The War Room" — a 4-panel strategic command center view for "Bots in a Field". Built on v11 (same game logic), this version transforms the single-camera observer experience into four synchronized panels: Tactical View (enhanced field with heat maps, threat ranges, pack territories), Relationship Web (force-directed graph of bot relationships), Population Timeline (scrolling stacked area chart), and Battle Cam (auto-directed cinematic camera). Tactical console design theme (cyan/amber accents), glassmorphism cards. Players configure their bot's AI before watching it compete autonomously.

## Architecture

Modular JS files with a 4-panel rendering architecture:

```
bots-strategy-v11.2/
├── index.html               # HTML structure, tabs, setup UI, 4-panel war-room grid
├── styles.css               # All styling including war-room grid layout
├── js/
│   ├── config.js            # Constants, state, behavior/rule definitions, lifecycleSettings
│   ├── log.js               # Simulation logging and output
│   ├── billboards.js        # Billboard system (ads near dot clusters)
│   ├── game.js              # YellowDot and Bot classes
│   ├── bot-ai.js            # Bot decision-making (simple/advanced/expert modes)
│   ├── bot-render.js        # Bot rendering with health bars, strategy badges, lifecycle effects
│   ├── ui.js                # Setup UI, tab navigation, strategy editors
│   ├── ui-strategy.js       # Strategy mode editors
│   ├── ui-lifecycle.js      # Lifecycle feature UI
│   ├── ui-settings.js       # Settings and import/export
│   ├── ui-notifications.js  # Event notification system
│   ├── debug.js             # Debug panel and target line overlay
│   ├── main.js              # Game loop, 4-panel rendering, camera, collision, heat map, overlays
...
│   ├── panel-relationship.js # Panel 2: Force-directed relationship graph
│   ├── panel-timeline.js    # Panel 3: Population timeline stacked area chart
│   └── panel-battlecam.js   # Panel 4: Auto-directed cinematic battle camera
