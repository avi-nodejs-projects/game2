// Bots Strategy v9 - Lifecycle System
...
function updateStarvation(bot) {
  ...
  if (cfg.scaling.enabled) {
    ...
    if (statDiff > 0) {
      const scaleFactor = 1 / (1 + statDiff * cfg.scaling.factor);
      effectiveThreshold = Math.max(60, effectiveThreshold * scaleFactor);
    }
  }
...
function applyStarvationStatDecay(bot) {
  ...
  const allStats = ['speed', 'attack', 'defence'];

  // Only consider stats that still have room to decay
  const decayable = allStats.filter(s => bot[s] > cfg.minStats[s]);
  if (decayable.length === 0) return null;

  let targetStat;
  switch (cfg.order) {
    case 'lowestFirst':
      targetStat = decayable.reduce((min, s) => bot[s] < bot[min] ? s : min, decayable[0]);
      break;
    case 'highestFirst':
      targetStat = decayable.reduce((max, s) => bot[s] > bot[max] ? s : max, decayable[0]);
      break;
    case 'random':
    default:
      targetStat = decayable[Math.floor(Math.random() * decayable.length)];
      break;
  }

  bot[targetStat] = Math.max(cfg.minStats[targetStat], bot[targetStat] - cfg.decayPerTick);
...
function resetBotLifecycleOnRespawn(bot) {
  ...
  // Clear speed boost (avoid decrement on a re-spawned bot)
