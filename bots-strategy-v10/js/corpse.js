// Bots Strategy v10 - Corpse System
...
function updateCorpses() {
  // Check expiration status once per corpse to avoid double update() calls
  const stillAlive = [];
  for (const c of corpses) {
    const isAlive = c.update();
    if (isAlive) {
      stillAlive.push(c);
    } else {
...
// Check if a bot can cannibalize a corpse based on cannibalism settings
// Note: The more complete canCannibalize function for live bots is in packs.js
// This is a simplified version specifically for corpse consumption
function canCannibalizeCorpse(bot, corpse) {
  ...
  switch (cfg.trigger) {
    case 'starving':
      if (!bot.isStarving) return false;
      break;
    case 'lowLives':
      if (bot.lives > cfg.lowLivesThreshold) return false;
      break;
    case 'always':
      // Always allow
      break;
    ...
    switch (cfg.packMembers) {
      case 'protected':
        ...
        if (cfg.cannibalOverride && lifecycleSettings.packs.cannibalism.enabled) {
          return canCannibalizeCorpse(bot, corpse);
        }
        return false;
      case 'food':
        return true;
...
function consumeCorpse(bot, corpse) {
  ...
  const statGainPerPoint = 1; // Each nutrition point = 1 stat point

  for (let i = 0; i < nutrition * statGainPerPoint; i++) {
